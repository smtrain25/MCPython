...
You receive the repo as-is. No need to copy any folder — your game lives in `games/game_template/`.

```
for_contributors/
├── ipe/                          # engine — DO NOT EDIT anything in here
│   ├── CONTRIBUTING.md
│   ├── SKILL.md                  # full engine API & design spec
│   ├── SETUP.md
│   └── ...
└── games/
    ├── game_template/            # YOUR GAME — edit files here
    │   ├── my_game.py            # game class (edit this)
    │   ├── run.py                # python games/game_template/run.py
    │   ├── verify.py             # solvability check
    │   ├── requirements.txt
    │   ├── play_logs/            # auto-created when you play in browser
    │   │   └── my_game_12345678_abc123.jsonl
    │   └── ...
    └── CST_01/                   # example game (run this first to verify setup)
        ├── game.py
        ├── run.py
        ├── verify.py
        ├── requirements.txt
        ├── play_logs/
        │   └── CST_01_87654321_xyz789.jsonl
        └── ...
```

**Do not modify any files inside `ipe/`.** The engine is shared infrastructure — your game code lives entirely in `games/game_template/`. If you need to change engine behaviour, override methods in your `BaseGame` subclass.

## Quick Start

```bash
# 1. Install dependencies (from repo root or from games/game_template)
pip install -r games/game_template/requirements.txt

# 2. Edit games/game_template/my_game.py — subclass BaseGame, override step() and on_set_level()

# 3. Run the game server
cd games/game_template
python run.py
...
The `run.py` and `verify.py` scripts handle path setup automatically — they find the `ipe/` engine in the repo.

---

## Try the Example Game (CST_01)

An example game — **Click to Clear** — is included at `games/CST_01/`. Use it to verify your setup and see a complete game before writing your own.

```bash
# From the for_contributors/ directory:

# 1. Install dependencies (shared with the engine)
pip install -r games/CST_01/requirements.txt

# 2. Run the game server
python games/CST_01/run.py
# Open http://127.0.0.1:5000 in your browser

# 3. Verify solvability across seeds
python games/CST_01/verify.py
```

CST_01 is a non-agentic click game: coloured squares are scattered on the grid, and you click each one to remove it. Clear all squares to advance. It demonstrates:

- Click-based input (`available_actions=[6]`)
- Grid scaling across 5 levels (8x8 → 64x64)
- Seeded random target placement
- Programmatic verification (since BFS can't enumerate clicks)
...
All commands run from inside your game directory (`games/game_template/`).
...
```json
{"_type": "session_start", "session_id": "CST_01_87654321_xyz789", "game_id": "CST_01", "timestamp": "2026-03-03T12:00:00Z"}
```

**Each action:**

```json
{"_type": "action", "session_id": "CST_01_87654321_xyz789", "timestamp": "...", "turn": 1, "level": 0, "action_id": 6, "action_data": {"x": 32, "y": 16}, "state_after": "NOT_FINISHED", "frame_hash": "a1b2c3d4e5f67890"}
```

**Game completion (written automatically when you beat all levels):**

```json
{"_type": "session_complete", "session_id": "CST_01_87654321_xyz789", "game_id": "CST_01", "game_completed": true, "total_turns": 47, "wall_clock_ms": 123456, "completion_token": "8a3f...long hex..."}
```

**You must play through your game at least once and complete all levels.** The play logs are required when you submit your game — they confirm the game is playable and record how many turns it took a human to finish. The `session_complete` record with `game_completed: true` is what we check.
